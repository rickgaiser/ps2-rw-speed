...
static inline void cdvdSt_read(void *buf)
{
    ...
    DPRINTF("cdvdSt_read: sectors=%lu\n", St->sectors);
...
static inline void cdvd_readchain(void *buf)
{
    ...
    for (i = 0, readpos = 0; i < 64; i++, ch++) {
		...
		DPRINTF("cdvd_readchain: lsn=%lu, sectors=%lu\n", ch->lsn, ch->sectors);
...
{ // Read Disc data to EE mem buffer
    ...
    GetSystemTime(&clk);
    SysClock2USec(&clk, &seconds, &usecs);
    DPRINTF("cdvd_readee: %lu: lsn=%lu, sectors=%lu\n", seconds, r->lsn, r->sectors);
